...
class StableDiffusionAdapterImg2ImgPipeline(DiffusionPipeline, LoraLoaderMixin):
    r"""
    ...
    def prepare_latents_img(self, image, timestep, batch_size, num_images_per_prompt, dtype, device, generator=None):
    ...
    def save_lora_weights(cls,
                          ...
                          custom_unet_lora_layers: Dict[str, Union[torch.nn.Module, torch.Tensor]] = None,
                          is_main_process: bool = True,
                          weight_name: str = None,
                          save_function: Callable = None,
                          safe_serialization: bool = True,
                          ):
        state_dict, custom_dict = {}, {}
        ...
        if custom_unet_lora_layers:
            custom_dict.update(pack_weights(custom_unet_lora_layers, "unet_custom"))
    ...
    def write_lora_layers(
            ...
            custom_dict: Dict[str, torch.Tensor],
            save_directory: str,
        ...
        os.makedirs(os.path.join(save_directory, "custom_duallora"), exist_ok=True)
        ...
        torch.save(custom_dict, os.path.join(save_directory, "custom_duallora", "duallora.bin"))
    ...
    def __call__(
        ...
        Returns:
            ...
            `tuple. When returning a tuple, the first element is a list with the generated imagges, and the second
        ...
        # ====================== add ===========================
        # 5. Preprocess image and generate sketch
        input_image = self.image_processor.preprocess(image)

        # ====================== add ===========================
        self.check_inputs(
            prompt, height, width, callback_steps, adapter_image, negative_prompt, prompt_embeds, negative_prompt_embeds
        )
        ...
        if isinstance(self.adapter, MultiAdapter):
            ...
            # raise ValueError(f"This file is for multi adapter, instead of {type(self.adapter).__name__}")
        ...
        latents = self.prepare_latents_img(
            input_image,
            latent_timestep,
            batch_size,
            num_images_per_prompt,
        ...
        # 8. Denoising loop
        if isinstance(self.adapter, MultiAdapter):
            num_adapter = len(self.adapter.adapters)
            adapter_conditioning_scale = [adapter_conditioning_scale/num_adapter] * num_adapter
...
class CustomMultiAdapterImg2ImgPipeline(DiffusionPipeline, LoraLoaderMixin):
    r"""
    Pipeline for text-to-image generation using Stable Diffusion augmented with T2I-Adapter
    https://arxiv.org/abs/2302.08453
    ...
    def __call__(
        ...
        # 5. Preprocess image and generate sketch
        input_image = self.image_processor.preprocess(image)
        image_list = [input_image, input_image]
        # ====================== add ===========================
        self.check_inputs(
            prompt, height, width, callback_steps, image_list, negative_prompt, prompt_embeds, negative_prompt_embeds
        )
        ...
        if isinstance(self.adapter, MultiAdapter):
            ...
            raise ValueError(f"This file is for multi adapter, instead of {type(self.adapter).__name__}")
            # adapter_input = _preprocess_adapter_image(image_list, height, width)
            # adapter_input = adapter_input.to(device=device, dtype=self.adapter.dtype)
...
class StableDiffusionAdapterPipeline(DiffusionPipeline, LoraLoaderMixin):
    r"""
    Pipeline for text-to-image generation using Stable Diffusion augmented with T2I-Adapter
    https://arxiv.org/abs/2302.08453
